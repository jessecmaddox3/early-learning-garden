...
const title={subitize:'How Many?',blend:'Robot Words',letters:'Letter Garden',path:'Bunny Hops'};
...
function remember(id){try{sessionStorage.setItem(`early-learning-garden-${game}-profile`,id);}catch{/* Explicit selection still works in this page. */}}
function remembered(){try{return sessionStorage.getItem(`early-learning-garden-${game}-profile`);}catch{return null;}}
...
export async function initProgress(which){
  ...
  try{store=await openProgressStore({dbName:`early-learning-garden-${game}-v1`,gameId:game,curriculumId:CURRICULA[game],normalize});}
  ...
  button(backups,'Export recovery copies',async()=>{const copies=await store.listRecovery(profile.id);download({app:'early-learning-garden-recovery',game,curriculum:CURRICULA[game],version:1,copies:copies.map(c=>({reason:c.reason,createdAt:c.createdAt,backup:exportSnapshot(game,profile.label,c.snapshot)}))},`early-learning-garden-${game}-recovery.json`);},{needsIdle:false});
  ...
  cloud=createCloudPanel(cloudRoot,{store,durable,normalize,gameId:game,curriculumId:CURRICULA[game],
    configURL:new URL('../cloud-config.local.json',document.baseURI),helpURL:'https://github.com/jessecmaddox3/early-learning-garden/blob/main/docs/cloud-setup.md',
    getProfile:()=>profile,isBusy:()=>pending>0||loading||retired||recoveryMode,onChange:refreshActive,
    onRestore:selectProfile,onProfilesCleared:async()=>{const remaining=await store.listProfiles();await selectProfile(remaining.find(p=>p.id===profile.id)||remaining[0]||await store.createProfile('Player 1',cleanProgress(game,{})));},onMessage:say,describe:s=>`${Object.values(s.items).reduce((n,x)=>n+x.attempts,0)} attempts; ${Object.values(s.items).filter(x=>x.mastered).length} practiced items`});
...
export function writeLocal(which,value){
  ...
  live=cleanProgress(game,value);const snapshot=structuredClone(live),id=profile.id;
  pending++;status();
