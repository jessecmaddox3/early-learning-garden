...
export async function initProgress(which){
  ...
  cloud=createCloudPanel(cloudRoot,{store,durable,normalize,gameId:game,curriculumId:CURRICULA[game],
    ...
    onRestore:selectProfile,onRemoval:removeAndSelect,onMessage:say,describe:s=>`${Object.values(s.items).reduce((n,x)=>n+x.attempts,0)} attempts; ${Object.values(s.items).filter(x=>x.mastered).length} practiced items`});
