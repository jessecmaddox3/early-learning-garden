(() => {
  ...
  async function setup() {
    ...
    const own = token;
    phase = "prompt";
    el2("gameplay").setAttribute("aria-busy", "true");
    hops = 0;
    el2("reaction").textContent = "";
    size = state.level === 1 ? 5 : 10;
    bunnyAt = state.level === 3 ? 2 + rand(size - 4) : 1;
    goal = bunnyAt + 1 + rand(Math.min(4, size - bunnyAt));
    draw();
    for (const [id, text] of [["bunny-on", "The bunny is on"], [`num-${bunnyAt}`, String(bunnyAt)], ["hop-to", "Hop to"], [`num-${goal}`, String(goal)]]) {
      await say(id, text);
      if (!life.valid(own)) return;
    }
    phase = "hopping";
    el2("gameplay").setAttribute("aria-busy", "false");
  }
  function draw(focus = false) {
    el2("path").replaceChildren();
    for (let n = 1; n <= size; n++) {
      ...
      b.dataset.n = n;
      b.textContent = n;
      b.setAttribute("aria-label", `${n}${n === bunnyAt ? ", bunny is here" : ""}${n === goal ? ", carrot" : ""}`);
      if (n === bunnyAt) b.insertAdjacentHTML("beforeend", '<span class="bunny" aria-hidden="true">\u{1F430}</span>');
      if (n === goal) b.insertAdjacentHTML("beforeend", '<span class="goal" aria-hidden="true">\u{1F955}</span>');
      if (n === bunnyAt + 1) b.classList.add("next");
      if (n < bunnyAt) b.classList.add("done");
      b.addEventListener("click", () => tapped(n));
      el2("path").append(b);
    }
    const next = el2("path").querySelector(`[data-n="${Math.min(bunnyAt + 1, goal)}"]`);
    if (focus) next?.focus({ preventScroll: true });
    const region = el2("path").parentElement, here = el2("path").querySelector(`[data-n="${bunnyAt}"]`), end = el2("path").querySelector(`[data-n="${goal}"]`);
    if (here && end) {
      const left = here.offsetLeft - el2("path").offsetLeft, right = end.offsetLeft - el2("path").offsetLeft + end.offsetWidth;
      if (right > region.scrollLeft + region.clientWidth) region.scrollLeft = right - region.clientWidth + 8;
      if (left < region.scrollLeft) region.scrollLeft = Math.max(0, left - 8);
    }
  }
  async function tapped(n) {
    if (phase !== "hopping" || !life.valid(token)) return;
    phase = "feedback";
    el2("gameplay").setAttribute("aria-busy", "true");
    const own = token;
    if (n !== bunnyAt + 1) {
      chime("soft");
      el2("path").querySelector(".next")?.classList.add("wobble");
      await say("next-one", "The next one.");
      if (life.valid(own)) {
        el2("path").querySelector(".next")?.classList.remove("wobble");
        phase = "hopping";
        el2("gameplay").setAttribute("aria-busy", "false");
      }
      return;
    ...
    draw(true);
    if (bunnyAt === goal) {
      state = score(GAME, state, `to${goal}`, true);
      save();
    }
    await say(`num-${bunnyAt}`, String(bunnyAt));
    if (!life.valid(own)) return;
    if (bunnyAt !== goal) {
      phase = "hopping";
      el2("gameplay").setAttribute("aria-busy", "false");
    ...
    chime("good");
    el2("reaction").textContent = "\u{1F955}";
    await say(`hops-${hops}`, `${hops} hops!`);
    if (!life.valid(own)) return;
    if (state.finished) return finish();
    life.later(own, setup, 700);
  }
  function finish() {
    phase = "finished";
    el2("reaction").textContent = "\u{1F3C6}";
    el2("again").setAttribute("aria-label", "Play another bunny session");
  ...
  el2("again").addEventListener("click", () => {
    ...
    el2("again").setAttribute("aria-label", "Start another bunny journey");
    setup();
  });
  installUnlock();
