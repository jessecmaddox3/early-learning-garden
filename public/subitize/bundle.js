(() => {
  ...
  var PATTERNS = { 1: [[50, 50]], 2: [[30, 30], [70, 70]], 3: [[26, 26], [50, 50], [74, 74]], 4: [[28, 28], [72, 28], [28, 72], [72, 72]], 5: [[26, 26], [74, 26], [50, 50], [26, 74], [74, 74]], 6: [[28, 22], [72, 22], [28, 50], [72, 50], [28, 78], [72, 78]] };
  function countPositions(level, n, rng = Math.random) {
    if (!Number.isInteger(n) || n < 1 || n > 6) throw new TypeError("Use one to six items.");
    if (level === 1) return PATTERNS[n].map((p) => [...p]);
    if (level === 3) {
      const left = Math.ceil(n / 2);
      return Array.from({ length: n }, (_, i) => [i < left ? 24 : 76, 20 + (i < left ? i : i - left) * 30]);
    }
    const cells = shuffled([[20, 25], [50, 25], [80, 25], [20, 75], [50, 75], [80, 75]], rng);
    return cells.slice(0, n).map(([x, y]) => [x + (Math.max(0, Math.min(1, rng())) - 0.5) * 4, y + (Math.max(0, Math.min(1, rng())) - 0.5) * 4]);
  }
  function numberChoices(answer, rng = Math.random) {
    const candidates = Array.from({ length: 7 }, (_, i) => i + 1).filter((n) => n !== answer);
    const near = candidates.filter((n) => Math.abs(n - answer) <= 2);
    return [answer, ...shuffled(near.length >= 2 ? near : candidates, rng).slice(0, 2)].sort((a, b) => a - b);
  }
  ...
  // public/subitize/subitize.js
  var GAME = "subitize";
  var FLASH_MS = 1150;
  var BUGS = ["\u{1F41E}", "\u{1F41D}", "\u{1F98B}", "\u{1F41B}", "\u{1F40C}"];
  ...
  function showBugs() {
    el2("jar").classList.remove("dark");
    el2("jar").replaceChildren();
    for (const [x, y] of question.positions) {
      const b = document.createElement("span");
      b.className = "bug";
      b.textContent = question.bug;
      b.style.cssText = `left:${x}%;top:${y}%;transform:translate(-50%,-50%)`;
      el2("jar").append(b);
    }
  }
  function flash(same = false) {
    ...
    const own = token;
    phase = "flash";
    el2("choices").hidden = true;
    el2("again").hidden = true;
    el2("reaction").textContent = "";
    if (!same) {
      const n = state.level === 1 ? 1 + rand(3) : state.level === 2 ? 1 + rand(4) : 5 + rand(2);
      question = { n, bug: BUGS[rand(BUGS.length)], positions: countPositions(state.level, n) };
    }
    showBugs();
    say("ask-how-many", "How many?");
    life.later(own, () => {
      el2("jar").classList.add("dark");
      el2("jar").innerHTML = '<div class="lid">\u{1FAD9}</div>';
      offer();
      phase = "answer";
    }, FLASH_MS);
  }
  function offer() {
    el2("choices").replaceChildren();
    for (const [i, n] of numberChoices(question.n).entries()) {
      const b = document.createElement("button");
      b.className = `big pick-${i + 1}`;
      b.type = "button";
      b.textContent = n;
      b.dataset.n = n;
      b.addEventListener("click", () => answered(n));
      el2("choices").append(b);
    }
    el2("choices").hidden = false;
    el2("again").hidden = false;
    el2("again").textContent = "\u{1F440}";
    el2("again").setAttribute("aria-label", "Look again at the same bugs");
  }
  async function answered(n) {
    if (phase !== "answer" || !life.valid(token)) return;
    phase = "feedback";
    const own = token, correct = n === question.n;
    state = score(GAME, state, `n${question.n}`, correct);
    save();
    el2("choices").hidden = true;
    el2("again").hidden = true;
    if (correct) {
      ...
      await say(`num-${question.n}`, `${question.n}!`);
      if (!life.valid(own)) return;
      if (state.finished) return finish();
      life.later(own, () => flash(), 700);
    } else {
      chime("soft");
      el2("reaction").textContent = "\u{1F914}";
      await say("lets-count", "Let's count them.");
      if (!life.valid(own)) return;
      showBugs();
      const bugs = [...el2("jar").children];
      bugs.forEach((b) => b.hidden = true);
      for (let i = 0; i < bugs.length; i++) {
        bugs[i].hidden = false;
        await say(`num-${i + 1}`, String(i + 1));
        if (!life.valid(own)) return;
      }
      await say(`total-${question.n}`, `${question.n} altogether.`);
      if (life.valid(own)) life.later(own, () => flash(), 500);
    }
  }
  function finish() {
    phase = "finished";
    el2("jar").classList.remove("dark");
    el2("jar").innerHTML = '<div class="lid">\u{1F31F}</div>';
    el2("choices").hidden = true;
    el2("reaction").textContent = "\u{1F44F}";
    el2("again").hidden = false;
    el2("again").textContent = "\u{1F501}";
    el2("again").setAttribute("aria-label", "Play another counting session");
  ...
  function load() {
    ...
    el2("reaction").textContent = "";
    el2("jar").classList.remove("dark");
    el2("jar").innerHTML = '<button class="lid start-jar" type="button" aria-label="Tap the jar to start">\u{1FAD9}</button>';
    el2("jar").querySelector("button").addEventListener("click", () => {
      if (phase === "start") flash();
    });
    say("ask-tap-jar", "Tap the jar to start.");
  }
  el2("again").addEventListener("click", () => {
    if (!life.valid(token)) return;
    if (phase === "answer") flash(true);
  ...
  initProgress(GAME).then(() => {
    ...
    el2("reaction").textContent = e.message;
  });
})();
