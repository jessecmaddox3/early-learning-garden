(() => {
  ...
  function createCloudConnection(config, {
    ...
    const guardedFetch = async (input, init = {}) => {
      assertActive();
      const target2 = new URL(typeof input === "string" || input instanceof URL ? input : input.url);
      if (target2.origin !== config.backend || !/^\/(auth|rest)\/v1\//.test(target2.pathname)) {
  ...
  // public/letters/letters.js
  var GAME = "letters";
  var FLOWERS = ["\u{1F33B}", "\u{1F337}", "\u{1F33A}"];
  ...
  function newRound() {
    ...
    const pool = WAVES.slice(0, state.wave + 1).flat(), weak = Math.min(...pool.map((l) => state.items[l]?.streak || 0)), weakest = pool.filter((l) => (state.items[l]?.streak || 0) === weak);
    target = weakest[Math.floor(Math.random() * weakest.length)];
    const n = (state.items[target]?.streak || 0) === 0 ? 2 : 3, opts = shuffled([target, ...shuffled(pool.filter((l) => l !== target)).slice(0, n - 1)]);
    el2("blooms").replaceChildren();
    for (const [i, l] of opts.entries()) {
      const b = document.createElement("button");
      b.type = "button";
      b.className = "bloom grown";
      b.dataset.l = l;
      b.setAttribute("aria-label", l);
      b.innerHTML = `<span class="petals">${FLOWERS[i]}</span><span class="letter">${l}</span>`;
      b.addEventListener("click", () => chose(l));
      el2("blooms").append(b);
    }
    say(`snd-${target}`, stretch(target));
    if (!document.querySelector("dialog")?.open) el2("speaker").focus({ preventScroll: true });
  }
  async function chose(l) {
    if (phase !== "answer" || !life.valid(token)) return;
    phase = "feedback";
    const own = token, asked = target, correct = l === asked;
    state = score(GAME, state, asked, correct);
    save();
    if (correct) {
      chime("good");
      el2("reaction").textContent = ANCHORS[asked][1];
      await say(`anchor-${asked}`, `${ANCHORS[asked][0]}.`);
      if (!life.valid(own)) return;
      if (state.finished) return finish();
      life.later(own, newRound, 650);
    } else {
      chime("soft");
      el2("reaction").textContent = "\u{1F914}";
      await say(`snd-${l}`, stretch(l));
      if (!life.valid(own)) return;
      await say(`snd-${asked}`, stretch(asked));
      if (life.valid(own)) phase = "answer";
    }
  }
  function finish() {
    phase = "finished";
    el2("reaction").textContent = "\u{1F308}";
    el2("blooms").replaceChildren();
  ...
  el2("speaker").addEventListener("click", () => {
    if (phase !== "answer" || !life.valid(token)) return;
    cancelAudio();
    say(`snd-${target}`, stretch(target));
    if (!document.querySelector("dialog")?.open) el2("speaker").focus({ preventScroll: true });
  ...
  el2("again").addEventListener("click", () => {
    if (phase !== "finished" || !life.valid(token)) return;
