    ...
    async function renderConnected() {
      ...
      if (selected) {
        ...
        button2(`Save ${selected.label} as a new cloud learner`, async () => {
          ...
          if (!alive(original, token2)) return;
          const result = await onMutation(() => store2.attach(selected.id, { backend: config.backend, ownerId: original.ownerId, profileId: remoteProfile.id }, current2.localRevision, null, "device"));
          if (!alive(original, token2)) return;
          await renderConnected();
