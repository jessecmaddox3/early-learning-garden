...
function endpoint() {
  ...
  api.fetch = async (input, init) => {
    ...
    api.row = { owner_id: api.owner, profile_id: P, game_id: 'blend', curriculum_id: 'test-v1', ...body, revision: (api.row?.revision ?? 0) + 1 }
...
async function connect(api = endpoint(), options = {}) {
  const connection = createCloudConnection(config, { gameId: 'blend', curriculumId: 'test-v1', normalize, loadSDK: () => import('@supabase/supabase-js'), nativeFetch: api.fetch, ...options })
...
test('disconnect during lazy import never constructs a client', async () => {
  ...
  const connection = createCloudConnection(config, { gameId: 'blend', curriculumId: 'test-v1', normalize, loadSDK: () => gate.promise })
  connection.disconnect()
...
test('the authenticated owner and full binding constrain all reads', async () => {
  ...
  assert.equal(request.searchParams.get('game_id'), 'eq.blend')
...
test('disconnect during REST cannot acknowledge the durable queue, even if the server committed', async () => {
  const { connection, api } = await connect()
  await login(connection)
  const store = await openProgressStore({ indexedDB: new IDBFactory(), gameId: 'blend', curriculumId: 'test-v1', normalize })
...
test('a committed blend save still reconciles after JSON object keys are reordered by the database',async()=>{
  const {cleanProgress,normalizeSnapshot}=await import('../public/shared/snapshot.js');
  const {connection,api}=await connect(endpoint(),{normalize:value=>normalizeSnapshot('blend',value)});
  try {
    await login(connection);
    const value=cleanProgress('blend',{items:{sun:{attempts:3,correct:3,streak:3},man:{attempts:1,correct:1,streak:1}}});
    ...
    // a different order from the original sequence in which words were practiced.
