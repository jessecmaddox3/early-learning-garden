...
6. Run `npm run build` and host `public/` over HTTPS. Keep the configuration at `public/cloud-config.local.json`, beside the picture menu. Each game reads that shared host configuration from its parent folder. Repository-subpath hosting is supported.
...
The standalone HTML files do not load cloud configuration or initialize the SDK. Hosted startup also makes no cloud request until the adult explicitly explores that option.
