...
def main():
    ...
    text = re.sub(r'(?m)^project_id\s*=.*$', 'project_id = "early-learning-synthetic-ci"', text)
    settings = {
        'api': {'enabled': True, 'port': 54321},
        'api.tls': {'enabled': True},
        'local_smtp': {'enabled': True, 'port': 54324},
        'auth': {'enabled': True, 'site_url': 'http://127.0.0.1:4197', 'enable_signup': True, 'enable_anonymous_sign_ins': False},
        ...
        'auth.email.template.confirmation': {'subject': 'Early Learning Garden synthetic sign-in code', 'content_path': './supabase/templates/otp.html'},
        'auth.email.template.magic_link': {'subject': 'Early Learning Garden synthetic sign-in code', 'content_path': './supabase/templates/otp.html'},
    }
