...
class Handler(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  if path=='/shared/progress-store.js':body=storewrap.encode();typ='text/javascript'
   ...
   if path=='/blend/':body=body.replace(b'<script src="./bundle.js" defer></script>',b'<script src="./blend.js" type="module"></script>')
...
try:
 with sync_playwright() as pw:
  ...
  page.add_init_script("window.reviewLog=[];localStorage.setItem('early-learning-sound-v1',JSON.stringify({muted:true}));")
  page.goto(origin+'/blend/')
  page.get_by_role('button',name='Learners and backups',exact=True).wait_for()
  page.locator('#again').click();page.locator('#rail').focus();page.locator('#rail').press('End');page.locator('#pics').wait_for(state='visible')
  word=page.locator('#wordOut').inner_text();page.locator(f'#pics [data-w="{word}"]').click()
  ...
  assert result['localRevision']==3 and result['remoteRevision']==1,result
  ...
  page.locator('#again').click();page.locator('#rail').focus();page.locator('#rail').press('End');page.locator('#pics').wait_for(state='visible')
  word=page.locator('#wordOut').inner_text();page.locator(f'#pics [data-w="{word}"]').click()
  page.wait_for_function("""async()=>{const r=await window.reviewStore.load((await window.reviewStore.listProfiles())[0].id);return r.snapshot.round===2&&r.remoteRevision===2&&!r.needsUpload}""")
  ...
  page.get_by_text('Robot Words learner: Player 1. Saved on this device.',exact=False).wait_for()
  page.locator('#again').click();page.locator('#rail').focus();page.locator('#rail').press('End');page.locator('#pics').wait_for(state='visible')
  word=page.locator('#wordOut').inner_text();page.locator(f'#pics [data-w="{word}"]').click()
  page.wait_for_function("""async()=>{const r=await window.reviewStore.load((await window.reviewStore.listProfiles())[0].id);return r.snapshot.round===1&&!r.binding}""")
  assert not errors,errors
