...
spec=importlib.util.spec_from_file_location('early_browser', ROOT/'scripts/test-saves-browser.py');early_browser=importlib.util.module_from_spec(spec);spec.loader.exec_module(early_browser)
settings=early_browser.settings
answer_ui=early_browser.answer_ui
READ_RECORDS = """() => new Promise((resolve,reject) => {
 const open=indexedDB.open('early-learning-garden-blend-v1');open.onerror=()=>reject(open.error);
...
def main():
    ...
    try:
        with tempfile.TemporaryFile() as log:
            ...
            for _ in range(50):
                try:
                    if request('GET', 'http://127.0.0.1:4197/cloud-config.example.json', app_key=False)[0] == 200: break
            ...
            with sync_playwright() as p:
                ...
                def new_page(label="Comet"):
                    ...
                    page.add_init_script("""
                      localStorage.setItem('early-learning-sound-v1',JSON.stringify({muted:true}));
                    ...
                    page.goto('http://127.0.0.1:4197/blend/')
                    ...
                    page.get_by_text('Robot Words learner: '+label+'.',exact=False).wait_for()
                    settings(page);page.get_by_role('button',name='Play as Player 1',exact=True).click();page.get_by_text('Robot Words learner: Player 1.',exact=False).wait_for()
                ...
                assert request('POST', api + '/rest/v1/learning_saves', {'owner_id':owner_b,'profile_id':saved_a['profile_id'],'game_id':'blend','curriculum_id':'words-12-v1','format_version':1,'snapshot':saved_b['snapshot'],'write_id':str(uuid.uuid4())}, token_b)[0] >= 400
                ...
                other.get_by_role('button', name='Restore as a new local learner', exact=True).click();other.get_by_text('Robot Words learner: Comet.',exact=False).wait_for()
                ...
                switching.get_by_role('button', name='Restore as a new local learner', exact=True).click();switching.get_by_text('Robot Words learner: Comet.',exact=False).wait_for()
                wait_record(switching, lambda r: r['binding'] is not None)
                settings(switching);switching.get_by_label('New learner nickname',exact=True).fill('Nebula');switching.get_by_role('button',name='Add learner',exact=True).click();switching.get_by_text('Robot Words learner: Nebula.',exact=False).wait_for()
                settings(switching)
                ...
                settings(switching)
                switching.get_by_role('button', name='Play as Comet', exact=True).click();switching.get_by_text('Robot Words learner: Comet.',exact=False).wait_for()
                ...
                settings(switching);switching.get_by_role('button', name='Play as Nebula', exact=True).click();switching.get_by_text('Robot Words learner: Nebula.',exact=False).wait_for()
                answer(switching)
                switching.wait_for_timeout(100)
